...
#    `functionName` is a function's name in your module (e.g., `predatorEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Ceres Barros/test/predator/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("predator")
  path <- list(modulePath = "C:/Users/Ceres Barros/test",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    predator = list(.saveInitialTime = NA)
  )
  ...
  if (exists("predatorEvent1", envir = .GlobalEnv)) {
    simOutput <- predatorEvent1(mySim)
  } else {
  ...
  if (exists("predatorEvent2", envir = .GlobalEnv)) {
    simOutput <- predatorEvent2(mySim)
  } else {
