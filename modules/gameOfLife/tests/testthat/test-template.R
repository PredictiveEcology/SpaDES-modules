...
#    `functionName` is a function's name in your module (e.g., `gameOfLifeEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/McIntire-lab/modules/gameOfLife/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("gameOfLife")
  path <- list(modulePath = "/home/achubaty/Documents/GitHub/McIntire-lab/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    gameOfLife = list(.saveInitialTime = NA)
  )
  ...
  if (exists("gameOfLifeEvent1", envir = .GlobalEnv)) {
    simOutput <- gameOfLifeEvent1(mySim)
  } else {
    simOutput <- mySim$gameOfLifeEvent1(mySim)
  }
  ...
  if (exists("gameOfLifeEvent2", envir = .GlobalEnv)) {
    simOutput <- gameOfLifeEvent2(mySim)
  } else {
    simOutput <- mySim$gameOfLifeEvent2(mySim)
  }
