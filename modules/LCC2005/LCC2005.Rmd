...
This is an example of a 'module group' or parent module called "LCC2005". 
It is a metamodule that points do the following child modules:
...
These modules can work together because their metadata are mutually compatible. 
These child modules actually have 2 different time units (month for caribou, year for others): the SpaDES package can fit these modules together in spite of this mismatch, because their time 
units are declared in their metadata. We will not look into the metadata in this file. That is for more advanced examples.
...
Here, we divide this initialization into two steps:
...
The SpaDES package runs dynamic modules. We have loaded the modules and input data into R, but now we have to load them into a simList object, which is required to run the dynamic simulations using a call to the spades function (the chunk following this one).
