...
If there is an ssh connection between machines, then a multi-machine PSOCK cluster can be used for running replicates of the model.
The master machine must have key-based authenticated ssh access to the slave machines, which must be running an ssh server (usually on a linux machine).
