...
Demography and dispersal movement of the wolf in the Italian Alps. This is an R port of the original SELES model that was used in [@marucco_2010].
...
## Using ssh on a group of Linux machines -- Advanced user

If you have a group of Linux machines, you can connect them using ssh and build clusters using all those machines, using PSOCK. To do this, the "slave" machines must be running an ssh server, and there is a stored ssh key on each of those machines, authenticating the connection from the master. If your desktop is a Windows machine, you will have to run these from a Linux machine (e.g., using Rstudio server running on one of the Linux machines, or running R from the command line from the master). 


#### Getting ssh running on Windows (only from a Windows Desktop)

Assuming that you have Rstudio server running on a Linux machine or can run R at a command prompt on a Linux machine, you will have to connect to the "master", create the ssh key pair while connected to that master, then copy the public key to each of the Linux "slaves". 
...
#### Create ssh key pairs
  ...
  - (if Windows) Connect to Linux machine master via ssh, e.g., 'ssh user@machine'
...
This model is a rewrite of a version originally written in SELES and used in the publication, [@marucco_2010]. This makes use of parallel package. The data files represent 100 replicate simulation outputs from the original SELES code.
