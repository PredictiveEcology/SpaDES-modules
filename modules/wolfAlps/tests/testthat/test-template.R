...
#    `functionName` is a function's name in your module (e.g., `wolfAlpsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Sarah/GitHub/McIntire-lab/Wolf/wolfAlps/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("wolfAlps")
  path <- list(modulePath = "C:/Sarah/GitHub/McIntire-lab/Wolf",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    wolfAlps = list(.saveInitialTime = NA)
  )
  ...
  if (exists("wolfAlpsEvent1", envir = .GlobalEnv)) {
    simOutput <- wolfAlpsEvent1(mySim)
  } else {
    simOutput <- mySim$wolfAlpsEvent1(mySim)
  }
  ...
  if (exists("wolfAlpsEvent2", envir = .GlobalEnv)) {
    simOutput <- wolfAlpsEvent2(mySim)
  } else {
    simOutput <- mySim$wolfAlpsEvent2(mySim)
  }
