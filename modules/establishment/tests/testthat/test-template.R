...
#    `functionName` is a function's name in your module (e.g., `establishmentEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Eliot/SpaDES/modules/establishment/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("establishment")
  ...
  parameters <- list(
    ...
    establishment = list(.saveInitialTime = NA)
  )
  ...
  if (exists("establishmentEvent1", envir = .GlobalEnv)) {
    simOutput <- establishmentEvent1(mySim)
  } else {
    simOutput <- mySim$establishmentEvent1(mySim)
  }
  ...
  if (exists("establishmentEvent2", envir = .GlobalEnv)) {
    simOutput <- establishmentEvent2(mySim)
  } else {
    simOutput <- mySim$establishmentEvent2(mySim)
  }
