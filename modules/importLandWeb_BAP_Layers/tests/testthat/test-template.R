...
#    `functionName` is a function's name in your module (e.g., `importLandWeb_BAP_LayersEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Eliot/SpaDES/modules/importLandWeb_BAP_Layers/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("importLandWeb_BAP_Layers")
  path <- list(modulePath = "C:/Eliot/SpaDES/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    importLandWeb_BAP_Layers = list(.saveInitialTime = NA)
  )
  ...
  if (exists("importLandWeb_BAP_LayersEvent1", envir = .GlobalEnv)) {
    simOutput <- importLandWeb_BAP_LayersEvent1(mySim)
  } else {
    simOutput <- mySim$importLandWeb_BAP_LayersEvent1(mySim)
  }
  ...
  if (exists("importLandWeb_BAP_LayersEvent2", envir = .GlobalEnv)) {
    simOutput <- importLandWeb_BAP_LayersEvent2(mySim)
  } else {
    simOutput <- mySim$importLandWeb_BAP_LayersEvent2(mySim)
  }
